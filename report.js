...
// Função principal para enviar o report
async function enviarReport(event) {
    // Previne o comportamento padrão do formulário
    event.preventDefault();
    
    const botao = document.getElementById('btn-enviar-reports');
    const textoBotao = botao.querySelector('.btn-text');
    const loader = botao.querySelector('.btn-loader');
    ...
    try {
        ...
        const resposta = await enviarParaAPI(dadosFormulario);
        
        // Sucesso
        botao.classList.remove('loading');
        botao.classList.add('success');
        textoBotao.textContent = ' Reporte Enviado!';
        textoBotao.style.opacity = '1';
        textoBotao.innerHTML = '<i class="fas fa-check"></i> Reporte Enviado!';
        
        // Redirecionar após sucesso
        setTimeout(() => {
            window.location.href = 'index.html?report=success';
        }, 1500);
        ...
        textoBotao.style.opacity = '1';
        textoBotao.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Erro ao Enviar';
        loader.style.display = 'none';
        
        // Restaurar botão após 3 segundos
        setTimeout(() => {
            botao.disabled = false;
            botao.classList.remove('error', 'loading');
            textoBotao.innerHTML = '<i class="fas fa-paper-plane"></i> Enviar Reporte';
            textoBotao.style.opacity = '1';
        }, 3000);
        
        mostrarErro('Erro ao enviar reporte. Tente novamente.');
    }
}
...
function coletarDadosFormulario() {
    return {
        ...
        urgencia: document.querySelector('#urgencia')?.value,
        timestamp: new Date().toISOString(),
        usuario: obterUsuarioLogado() // Sua função para obter usuário
    };
}

// Simulação de envio para API (substitua pela sua implementação real)
...
function mostrarErro(mensagem) {
    // Você pode implementar um toast ou alerta estilizado
    alert(mensagem); // Substitua por um sistema de notificação melhor
}

// Obter usuário logado (exemplo)
function obterUsuarioLogado() {
    return localStorage.getItem('usuario') || 'Anônimo';
}

// Prevenir envio duplo
let enviando = false;
function enviarReport(event) {
    if (enviando) return;
    enviando = true;
    
    // ... resto do código
    
    // Resetar flag no final
